...
const NotificationItem = ({ notification, onNotificationClick, onDeleteNotification }) => {
  ...
  return (
    ...
    >
      <ListItemAvatar>
        {/* {avatar} */}
        {/* <Iconify icon={avatar} sx = {{width:40, height:40}}/> */}
      </ListItemAvatar>
      <ListItemText
        primary={
          <Typography
...
function HeaderNotifications() {
  ...
  const handleMarkAllAsRead = async () => {
    try {
      await axios.post(
        `${backendURL}/Educator/setNotificationSeen`,
  ...
  const handleDeleteNotification = async (id) => {
    try {
      await axios.delete(`${backendURL}/Educator/deleteNotification`, {
        data: {
          user_id: notifications[0].id,
  ...
  React.useEffect(() => {
    const fetchNotifications = async () => {
      try {
        ...
        const response = await axios.get(backendURL+'/Educator/getNotifications', {
          headers: {
            "shiksha-niyojak": user_token,
          },
        });
