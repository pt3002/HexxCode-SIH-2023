...
function HeaderNotifications() {
  ...
  const handleMarkAllAsRead = async () => {
    try {
      await axios.post(
        `${backendURL}/CurriculumDeveloper/setNotificationSeen`,
  ...
  const handleDeleteNotification = async (id) => {
    try {
      await axios.delete(`${backendURL}/CurriculumDeveloper/deleteNotification`, {
        data: {
          user_id: notifications[0].id,
  ...
  React.useEffect(() => {
    const fetchNotifications = async () => {
      try {
        ...
        const response = await axios.get(backendURL+'/CurriculumDeveloper/getNotifications', {
          headers: {
            "shiksha-niyojak": user_token,
          },
        });
