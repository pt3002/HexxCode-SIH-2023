...
export default function AddGuidelines() {
    ...
    if (fileData) {
      ...
      axios.post(URL, fileData).then(function (response) {
        if (response && response.data && response.data.filename) {
          ...
          axios
            .post(backendURL + "/AICTEAdmin/addGuideline", body)
            .then((res) => {
              if (res.data.message) {
                Swal.fire({
                  icon: "success",
    ...
    if (fileData) {
      ...
      axios.post(URL, fileData).then(function (response) {
        if (response && response.data && response.data.filename) {
          ...
          axios
            .post(backendURL + "/AICTEAdmin/updateGuideline", body)
            .then((res) => {
              console.log(res);
              if (res.data.message) {
                Swal.fire({
      ...
      let body = {
        id: guidelineId,
        title: newGuideline.title,
        mongo_file_id: newGuideline.mongo_file_id,
        last_modified_date: new Date(),
      };
  ...
  const handleClickDelete = (params) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        axios
          .post(backendURL + "/AICTEAdmin/deleteGuideline", body)
          .then((res) => {
            Swal.fire({
              icon: "success",
              title: "SUCCESS",
  ...
  React.useEffect(() => {
    axios.get(backendURL + "/AICTEAdmin/getAllGuidelines").then((res) => {
  ...
  return (
    <>
      ...
      <Dialog open={open} onClose={handleClose}>
        <DialogTitle>
          {Edit ? "EDIT GUIDELINE" : "ADD NEW GUIDELINE"}
        </DialogTitle>
        <DialogContent>
          <TextField
            autoFocus
